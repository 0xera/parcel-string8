...
class ReadStringsParcelableBenchmark {
    ...
    private fun readStringsBenchmark(count: Int) {
        benchmarkRule.measureRepeated {
            runWithTimingDisabled {
                if (File(context.filesDir, "strings-parcelable-${count}x").exists().not()) {
                    val data = ParcelableStringsData(generateStringsList(count))
                    data.writeParcelable(context, "strings-parcelable-${count}x")
                }
            }
            readParcelable<ParcelableStringsData>(context, "strings-parcelable-${count}x")
        }
    }
}